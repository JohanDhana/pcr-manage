...
class tests extends CI_Controller
{
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->event_model->update_tests($id, $post_image);
			// Set message
	...
	public function reservations($id)
	{
		...
		$config["base_url"] = base_url() . "tests/" . $id . "/reservations";
	...
	public function list()
	{
		...
		$config["total_rows"] = $this->event_model->count_tests()['COUNT(*)'];
		$this->pagination->initialize($config);
		$offset = ($this->input->get('page')) ? (($this->input->get('page') - 1) * $config["per_page"]) : 0;
		$data["links"] = $this->pagination->create_links();
		$this->load->view('templates/header');
		$data['eventList'] = $this->event_model->get_tests($config["per_page"], $offset);
